...
def phrase_extractor(sent, word_weight, stopwords=stopwords_zhTW):
    ...
    key_phrases = jio.keyphrase.extract_keyphrase(
        zhcn_sent, specified_words=zhcn_word_weight
    )
...
def week_text(x, min_year):
    ...
    start = dt.datetime.strftime(
        dt.datetime.strptime(f"{year}-W{x}-1", "%Y-W%W-%w"), "%Y%m%d"
    )
    end = dt.datetime.strftime(
        dt.datetime.strptime(f"{year}-W{x}-0", "%Y-W%W-%w"), "%Y%m%d"
    )
...
def gen_kw_phrase(df, msg_col, word_weight):
    ...
    for sent in tqdm(sent_list):
        phrase_list.append(
            phrase_extractor(
                sent=str(sent), word_weight=word_weight, stopwords=stopwords_zhTW
            )
        )
...
def gen_time_durations(df, msg_col):
    ...
    """
    df["date"] = df["date"].astype(str).apply(lambda x: str(x).replace("-", ""))
    df["month"] = df["date"].apply(lambda x: str(x)[:6])
    min_year = int(df["date"].min()[:4])
    ...
    df["weekOfYear"] = df["weekOfYear"].apply(lambda x: week_text(int(x), min_year))
...
def make_stacked(df, kw_col, drop_col, msg_col):
    df_phrase_stacked = df.drop(kw_col, axis=1).join(
        df[kw_col]
    ...
    df_phrase_stacked[kw_col] = df_phrase_stacked[kw_col].apply(
        lambda x: kw_cleasner(x)
    )
    df_phrase_stacked = df_phrase_stacked[
        (df_phrase_stacked[kw_col].notnull()) & (df_phrase_stacked[kw_col] != "")
...
def gen_kw_stats_table(df_phrase_stacked, duration_col, kw_col):
    ...
    df_duration = (
        df_duration.groupby([duration_col, kw_col])
        .agg({kw_col: "count"})
        .rename(columns={kw_col: "count"})
    )
    # count
    ...
    for i in tqdm(range(df_duration_pivot_perc.shape[0])):
        df_duration_pivot_perc.iloc[i] = (
            df_duration_pivot.iloc[i] / df_duration_pivot.iloc[i].sum() * 100
        )
    ...
    for _set in loop_set:
        if _set[1] == "count":
            df_duration_melt = _set[0].melt(
                id_vars=[duration_col], var_name=kw_col, value_name=_set[1]
            )
        else:
            df_duration_melt[_set[1]] = _set[0].melt(
                id_vars=[duration_col], var_name=kw_col, value_name=_set[1]
            )[_set[1]]
    df_duration_melt = df_duration_melt[df_duration_melt["count"] > 0].reset_index(
        drop=True
    )
...
def gen_kw_stats_only_new(df_phrase_stacked, duration_cols, kw_col):
    ...
    for i in tqdm(range(len(duration_cols))):
        ...
        df_duration_pivot_cumsum["first_show"] = df_duration_pivot_cumsum.apply(
            lambda row: check(row, time_slot), axis=1
        )
        ...
        for slot in tqdm(time_slot):
            ...
            df_slot = df_duration_pivot_cumsum[
                df_duration_pivot_cumsum["first_show"] == slot
            ]
    ...
    for key_col in new_phrases.keys():
        for time in tqdm(new_phrases[key_col].keys()):
            tmp = pd.DataFrame(
                new_phrases[key_col][time], columns=["關鍵詞", "實際次數", "佔比"]
            )
            tmp[key_col] = time
...
def count_concurrency(afa_kw_corpus):
    ...
    afa_kw_corpus_unique_counts = dict(
        sorted(afa_kw_corpus_unique_counts.items(), key=lambda k: k[1], reverse=True)
    )

    kw_concurrency_sorted = dict()
    for kw in tqdm(afa_kw_corpus_unique_counts.keys()):
        sorted_temp_dict = dict(
            sorted(kw_concurrency[kw].items(), key=lambda k: k[1], reverse=True)
        )
