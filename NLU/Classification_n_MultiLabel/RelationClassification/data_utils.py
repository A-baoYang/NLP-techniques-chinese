...
class MyTokenizer(object):
    def __init__(self, pretrained_model_path=None, mask_entity=False):
        self.pretrained_model_path = pretrained_model_path or 'bert-base-chinese'  # bert-base-multilingual-cased
...
class SentenceREDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample_tag_ids = [self.tag2idx[sample_tag] for sample_tag in sample_tags]
        sample_tag_ids = [1 if i in sample_tag_ids else 0 for i in range(len(self.tag2idx))]
        ...
        sample = {
            ...
            'tag_ids': torch.tensor(np.array(sample_tag_ids).astype(np.float32).tolist())
        }
        return sample
