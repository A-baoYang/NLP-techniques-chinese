...
input_filepath = os.path.join(
    args.root_dir, args.data_dir, "BANK", "cathaybk_0513.xlsx"
)
#%%
...
# df["EXPECT_TYPE_CODE"] = df["EXPECT_TYPE_CODE"]
# filepath = os.path.join(
#     args.root_dir, args.output_dir, 
#     f"afa_chatbot_records-BANK-{min_date}_{max_date}.csv")
...
for duration_col in tqdm(duration_cols):
    ...
    _df["growth_rate"] = np.where(
        _df["percentage"]==_df["growth_rate"], 
        np.nan, _df["growth_rate"]
    )
    print(_df.head())
...
filepath = os.path.join(
    args.root_dir, args.output_dir, 
    f"afa_BANK-{kw_col}-stats-original-{min_date}_{max_date}.xlsx")
...
filepath = os.path.join(
    args.root_dir, args.output_dir, 
    f"afa_BANK-{kw_col}-stats-new-{min_date}_{max_date}.xlsx")
with pd.ExcelWriter(filepath) as writer:
    for i in tqdm(range(len(sheet_names))):
...
filepath = os.path.join(
    args.root_dir, args.output_dir, 
    f"sorted_cooccurrence-{kw_col}.json")
